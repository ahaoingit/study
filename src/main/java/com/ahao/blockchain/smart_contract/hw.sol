...
contract Score{
    ...
    //清算积分
    function LiquidationScore() public{
        for(uint i = 0;i<customers.length;i++){
            settledScoreAmount +=customer[customers[i]].scoreAmount;
            customer[customers[i]].scoreAmount=0;
        }
        for(uint j = 0;j<merchants.length;j++){
            settledScoreAmount +=merchant[merchants[j]].scoreAmount;
            merchant[merchants[j]].scoreAmount=0;
        }
    }
}
